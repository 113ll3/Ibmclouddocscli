...
Oui, vous recevez des notifications relatives aux nouvelles éditions de l'interface de ligne de commande dès qu'elles sont disponibles. Abonnez-vous au [référentiel d'éditions d'interface de ligne de commande {{site.data.keyword.cloud_notm}}](https://github.com/IBM-Cloud/ibm-cloud-cli-release/releases/){: external}
...
Les applications déployées dans une chaîne d'outils DevOps peuvent également inclure des fichiers, tels que `toolchain.yml` et `pipeline.yml`. Les applications déployées manuellement peuvent inclure des fichiers de charte Helm et un fichier `manifest.yml` (pour le déploiement dans Cloud Foundry ou Kubernetes, par exemple).
