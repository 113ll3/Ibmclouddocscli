...
# Utilisation du {{site.data.keyword.dev_cli_notm}}
{: developing}

Le développement d'applications natives en cloud à l'aide du plug-in {{site.data.keyword.dev_cli_notm}} est un processus relativement simple :
...
- Utiliser la commande [`bx dev enable`](./commands.html#enable) de l'interface de ligne de commande {{site.data.keyword.dev_cli_notm}} afin de rapidement activer pour le cloud une application côté serveur existante. 
...
- La commande [`code`](./commands.html#enable) vous permet d'extraire le code source généré d'une application directement sur votre poste de travail. 
...
- Vidéo : [Installation d'IDT sous Ubuntu Linux]()
...
d'entrée démarrant votre application est défini. Quand vous choisissez d'exécuter votre application via le plug-in {{site.data.keyword.dev_cli_short}}, ce conteneur est utilisé. Le fichier Dockerfile pour ce conteneur est défini par le paramètre [`dockerfile-run`](commands.html#run-parameters).
