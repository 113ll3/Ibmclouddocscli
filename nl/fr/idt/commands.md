...
# Commandes {{site.data.keyword.dev_cli_notm}} (bx dev)
{: #idt-cli}

Version : 1.2.0
Publication : 8 mars 2018

Utilisez les commandes {{site.data.keyword.dev_cli_notm}} (bx dev) suivantes pour créer un projet, le déployer, le déboguer et le tester :
...
- La commande [view](#view) vous permet d'afficher l'URL déployée de l'application à des fins de test et de visualisation. 
...
Vous pouvez éventuellement indiquer un nom de projet ; l'interface de ligne de commande ignore la mise en correspondance basée sur le nom de dossier/d'application. Dans ce cas, l'interface de ligne de commande ouvre la console du projet nommé dans un navigateur Web.   
...
génération doit d'abord être effectuée pour le projet à l'aide de la commande build avec l'argument `--debug`.
...
Vous pouvez effectuer une requête relative au statut des conteneurs qui sont utilisés par le plug-in {{site.data.keyword.dev_cli_short}}, comme défini par `container-name-run` et `container-name-tools`.
...
génération doit d'abord être effectuée pour le projet à l'aide de la commande `build --debug`.
Le conteneur tools est ensuite utilisé afin d'appeler `test-cmd` pour l'application.
