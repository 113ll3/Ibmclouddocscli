...
<table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 3. Comandos dns do Softlayer</caption>
 <thead>
...
<table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 4. Endereços IP globais do Softlayer</caption>
 <thead>
 <th colspan="6">Endereços IP globais do Softlayer</th>
 </thead>
 <tbody>
 <tr>
...
## Comandos de imagem do {{site.data.keyword.BluSoftlayer_notm}}
 ...
 <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 5. Comandos de imagem do Softlayer</caption>
 <thead>
...
## Comandos de armazenamento ISCSI anterior do {{site.data.keyword.BluSoftlayer_notm}}

  <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 6. Comandos de armazenamento ISCSI anterior do SoftLayer</caption>
 <thead>
 <th colspan="6">Comandos de armazenamento ISCSI anterior do SoftLayer</th>
 </thead>
 <tbody>
 <tr>
...
## Comandos de segurança do {{site.data.keyword.BluSoftlayer_notm}}
 ...
 <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 7. Comandos de segurança do Softlayer</caption>
 <thead>
...
## Comandos de sub-rede do {{site.data.keyword.BluSoftlayer_notm}}
 ...
 <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 8. Comandos de sub-rede do Softlayer</caption>
 <thead>
...
## Comandos de servidor virtual do {{site.data.keyword.BluSoftlayer_notm}}
 ...
 <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 9. Comandos de servidor virtual do SoftLayer</caption>
 <thead>
...
## Comandos de VLAN do {{site.data.keyword.BluSoftlayer_notm}}
  ...
  <table summary="Comandos gerais do Softlayer ordenados alfabeticamente que possuem links que levam a mais informações sobre o comando">
<caption>Tabela 10. Comandos de VLAN do Softlayer</caption>
 <thead>
...
<strong>Opções de comando</strong>:
   ...
   <dd>URL do terminal da API do Softlayer, o padrão é: https://api.softlayer.com/rest/v3.1</dd>
   <dt>-u,--sl-user</dt>
   <dd>Nome do usuário do Softlayer</dd>
   <dt>-p,--sl-password</dt>
   <dd>Senha do Softlayer ou chave API</dd>
   <dt>-c,--account-id</dt>
...
Visualizar informações da ajuda de todos os comandos para operar o ambiente do Softlayer.
```
bluemix sl help
...
Autorizar os hosts a acessarem um determinado volume.
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--sortby</dt>
   <dd>Coluna pela qual classificar, as opções são: `id`, `name`,  `type`, `private_ip_address`, `host_iqn`, `username` ou `password`.</dd>
   <dt>--columns</dt>
   <dd>  Colunas para exibição, as opções são: `id`, `name`, `type`, `private_ip_address`, `host_iqn`, `username` ou `password`.</dd>
</dl>
...
```
Esse comando lista todos os hosts que têm autorização para acessar o volume com ID `12345678` e os classifica pelo ID.

### bluemix sl block access-revoke
{: #sl_block_access_revoke}

Revogar a autorização de hosts que acessam um determinado volume.
```
...
```
Esse comando revoga o acesso de servidor virtual com ID `87654321` para o volume com ID `12345678`.

### bluemix sl block replica-failback
{: #sl_block_replica_failback}

Causar failback de um volume de bloco por meio da réplica.
```
...
Causar failover de um volume de bloco no volume de réplica especificado.
```
...
<strong>Opções de comando</strong>:
   ...
   <dd>Requerido. Planejamento da captura instantânea a ser usado para replicação. As opções são: `HOURLY`, `DAILY` ou `WEEKLY`.</dd>
   <dt>-d, --datacenter</dt>
   <dd>Requerido. Nome abreviado do data center da réplica. Por exemplo, `dal09`.</dd>
   <dt>--tier</dt>
   <dd>Opcional. Camada de armazenamento de resistência (IOPS por GB) do volume primário para o qual uma réplica é pedida. As opções são: `0.25`, `2`, `4` ou `10`.</dd>
   <dt>--os-type</dt>
   <dd>Opcional. Tipo de sistema operacional do volume primário para o qual uma réplica é pedida. As opções são: `HYPER_V`, `LINUX`, `VMWARE`, `WINDOWS_2008`, `WINDOWS_GPT`, `WINDOWS` ou `XEN`.</dd>
   <dt>-f, --force</dt>
...
```
Esse comando pede uma réplica para o volume com ID `12345678`, que executa a replicação DIÁRIA, está localizado em dal09 com o nível de camada `4` e o OS tipo `Linux`.
...
<strong>Opções de comando</strong>:
   <dl>
   <dt> -n, --note</dt>
   <dd>Notas para configurar na nova captura instantânea</dd>
	</dl>
...
```
Esse comando cria uma captura instantânea para o volume com ID `12345678` e com a nota de adição como `snapshotforbluemix`.

### bluemix sl block snapshot-disable
{: #sl_block_snapshot_disable}

Desativar capturas instantâneas no planejamento especificado de um determinado volume.
```
...
<strong>Opções de comando</strong>:
   ...
   <dd>Requerido. O planejamento de captura instantânea: `HOURLY`, `DAILY` ou `WEEKLY`.</dd>
	</dl>
...
```
Esse comando desativa a captura instantânea diária do volume com ID `12345678`.

### bluemix sl block snapshot-enable
{: #sl_block_snapshot_enable}

Ativar capturas instantâneas de um determinado volume no planejamento especificado.
```
...
<strong>Opções de comando</strong>:
   ...
   <dt>-d, --day-of-week</dt>
   <dd>Dia da semana em que as capturas instantâneas devem ser obtidas, número inteiro entre 0 e 6.
0 significa domingo, 1 significa segunda-feira, 2 significa terça-feira, 3 significa quarta-feira, 4 significa quinta-feira, 5 significa sexta-feira, 6 significa sábado.</dd>
	</dl>
...
```
Esse comando ativa a captura instantânea do volume com ID `12345678`. A captura instantânea é obtida às 2h todos os domingos e até 5 capturas instantâneas são retidas.
...
```
Esse comando exclui capturas instantâneas com ID `12345678`.

### bluemix sl block snapshot-list
{: #sl_block_snapshot_list}

Listar capturas instantâneas do armazenamento de bloco
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--sortby</dt>
   <dd>Coluna pela qual classificar. As opções são: `id`, `name`, `created` e `size_bytes`.</dd>
	</dl>
...
```
Esse comando lista todas as capturas instantâneas do volume com ID `12345678` e os classifica pelo ID.
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>-s, --size</dt>
   <dd>Requerido. O tamanho do espaço de captura instantânea a ser criado em GB.</dd>
   <dt>-t, --tier</dt>
...
```
Esse comando restaura o volume com ID `12345678` por meio da captura instantânea com ID `87654321`.
...
<strong>Opções de comando</strong>:
   ...
   <dd>Filtrar pelo tipo de volume de armazenamento. As opções são: `performance` e `endurance`.</dd>
   <dt>--order</dt>
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--storage-type</dt>
   <dd>Tipo de volume de armazenamento [necessário], as opções são: performance, endurance.</dd>
   <dt>--size</dt>
   <dd>Tamanho do volume de armazenamento em GB [necessário]</dd>
   <dt>--iops</dt>
   <dd>IOPs do armazenamento de desempenho, entre 100 e 6.000 em múltiplos de 100 [necessário para o desempenho de tipo de armazenamento]</dd>
   <dt>--tier</dt>
   <dd>Camada de armazenamento de resistência (IOP por GB) [necessária para a resistência de tipo de armazenamento], as opções são: 0,25, 2, 4, 10</dd>
   <dt>--os-type</dt>
   <dd>Sistema operacional [necessário], as opções são: HYPER_V, LINUX, VMWARE, WINDOWS_2008, WINDOWS_GPT, WINDOWS, XEN</dd>
   <dt>-d, --datacenter</dt>
   <dd>Nome abreviado do data center [necessário]</dd>
   <dt>--snapshot-size</dt>
   <dd>Parâmetro opcional para pedir espaço de captura instantânea junto ao armazenamento de bloco de resistência; especifica o tamanho (em GB) de espaço de captura instantânea a ser pedido</dd>
   <dt>-f, --force</dt>
...
```
Esse comando pede um volume de resistência com tamanho 500 GB, nível de camada 4 IOPS por GB, tipo de OS XEN, localizado em dal09 e tamanho do espaço de captura instantânea adicional 500 GB.
...
Importa a zona com base em um arquivo de zona BIND.
```
...
```
Esse comando importa zona e seus registros de recurso do arquivo: `~/blumix.net.txt`.

### bluemix sl dns record-add
{: #sl_dns_record_add}
Incluir registro de recurso.
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--ttl</dt>
   <dd>Valor TTL em segundos, como 86400  [padrão: 7200].</dd>
    </dl>
...
```
Esse comando inclui um registro A na zona: bluemix.net, seu host é `ftp`, os dados são `127.0.0.1` e ttl é 86.400 segundos.
...
```
Esse comando edita registros na zona `bluemix.net` com `12345678` e configura seus dados como `127.0.0.2` e ttl como 3600.

```
 bluemix sl dns record-edit bluemix.net --by-record kibana --ttl 3600
```
Esse comando edita registros na zona `bluemix.net` com o nome do host `kibana` e configura todo o seu ttl como 3600.
...
```
Esse comando lista todos os registros A na zona `bluemix.net`, filtrados pelo host `elasticsearch` e ttl 900 segundos.
...
```
Esse comando cria uma zona chamada `bluemix.net`.
...
```
Esse comando exclui uma zona chamada `bluemix.net`.
...
Zona de impressão e registros de recursos no formato BIND
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--v6</dt>
   <dd>Pedir um IP IPv6.</dd>
   <dt>--test</dt>
   <dd>Testar a ordem.</dd>
    <dt>-f, --force</dt>
   <dd>Criar um IP global sem confirmação.</dd>
    </dl>
...
```
Esse comando cancela um endereço IP com ID `12345678`.

### bluemix sl globalip-list
{: #sl_globalip_list}

Lista todos os IPs globais.
```
...
```
Esse comando edita a imagem com ID `12345678` e configura seu nome como `ubuntu16`, a nota como `testing` e a tag como `staging`.
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--name</dt>
   <dd>Filtrar no nome da imagem.</dt>
   <dd>--public</dt>
...
Cancelar um volume iSCSI existente
```
...
<strong>Opções de comando</strong>:
   ...
   <dd>Um motivo opcional para cancelamento.</dd>
   <dt>--immediate</dt>
   <dd>Cancela o iSCSI imediatamente, em vez de no aniversário de faturamento.</dd>
   <dt>-f, --force</dt>
   <dd>Forçar a operação sem confirmação.</dd>
    </dl>
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--size</dt>
   <dd>Tamanho do volume iSCSI a ser criado (em gibibytes) [necessário]</dd>
   <dt>--datacenter</dt>
   <dd>Nome abreviado do data center [necessário]</dd>
	</dl>
...
Obter detalhes para um volume iSCSI
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--senha</dt>
   <dd>Mostrar credenciais para acessar o destino iSCSI.</dd>
   </dl>
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--order</dt>
   <dd>O ID do pedido que comprou esse armazenamento iscsi</dd>
   </dl>
...
Cancelar/Excluir a captura instantânea iSCSI
```
...
Criar uma captura instantânea de um volume iSCSI
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--notes</dt>
   <dd>Uma nota opcional para a captura instantânea.</dd>
   </dl>
...
Pedir espaço de captura instantânea para o volume iSCSI especificado
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>--capacity</dt>
   <dd>O tamanho do espaço de captura instantânea a ser criado.</dd>
   </dl>

### bluemix sl iscsi snapshot-list
{: #sl_iscsi_snapshot_list}

Listar capturas instantâneas de um volume iSCSI
```
...
<strong>Opções de comando</strong>:
   ...
   <dt>--csr</dt>
   <dd>Arquivo Certificate Signing Request</dd>
   <dt>--icc</dt>
...
As quantidades válidas variam por tipo.

 * Tipo    - Quantidades válidas (IPv4)
    ** público  - 4, 8, 16, 32
    ** privado - 4, 8, 16, 32, 64
 * Tipo    - Quantidades válidas (IPv6)
    ** público  - 64
...
Cancelar a instância do servidor virtual
```
...
<strong>Opções de comando</strong>:
   <dl>
   <dt>-n, --name</dt>
   <dd>Nome da imagem [necessário].</dd>
   <dt>--all</dt>
...
Criar instância do servidor virtual
```
...
<strong>Opções de comando</strong>:
   ...
   <dt>-c, --cpu</dt>
   <dd>Número de núcleos da CPU [necessário]</dd>
   <dt>-m, --memory</dt>
   <dd>Memória em megabytes [necessário]</dd>
   <dt>-d, --datacenter</dt>
   <dd>Nome abreviado do data center [necessário]</dd>
   <dt>-o, --os</dt>
   <dd>Código de instalação do OS. Dica: é possível especificar <OS>_LATEST</dd>
   <dt>--image</dt>
   <dd>ID da imagem. Veja: [bluemix sl image list](#bluemix_sl_image_list) para obter referência</dd>
   <dt>--billing</dt>
   <dd>Taxa de faturamento. O padrão é: de hora em hora. As opções são: de hora em hora, mensal</dd>
   <dt>--dedicated</dt>
   <dd>Criar um Servidor virtual dedicado (Nó privado)</dd>
   <dt>--san</dt>
   <dd>Usar armazenamento SAN em vez de disco local</dd>
   <dt>--test</dt>
   ...
   <dt>-k, --key</dt>
   <dd>Os IDs das chaves SSH a serem incluídas no usuário raiz (permitidas múltiplas ocorrências)</dd>
   <dt>--disk</dt>
   <dd>Tamanhos do disco (permitidas múltiplas ocorrências)</dd>
   <dt>--private</dt>
   <dd>Força o servidor virtual a ter acesso apenas à rede privada</dd>
   <dt>--like</dt>
   <dd>Usar a configuração de um servidor virtual existente</dd>
   <dt>-n, --network</dt>
   <dd>Velocidade da porta de rede em Mbps</dd>
   <dt>--tag</dt>
   <dd>Tags para incluir na instância (permitidas múltiplas ocorrências)</dd>
   <dt>-t, --template</dt>
   <dd>Um arquivo de modelo que padroniza as opções da linha de comandos</dd>
   <dt>-u, --userdata</dt>
   <dd>Sequência de metadados definida pelo usuário</dd>
   <dt>-F, --userfile</dt>
   <dd>Ler dados do usuário do arquivo</dd>
   <dt>--vlan-public</dt>
   ...
   <dt>--wait</dt>
   <dd>Aguarde até que o servidor virtual tenha concluído o fornecimento por até X segundos antes de retornar</dd>
   <dt>-f, --force</dt>
   <dd>Forçar operação sem confirmação</dd>
	</dl>
...
```
Esse comando pede uma instância de servidor virtual com o nome de host myvsi, o domínio bluemix.net, 4 núcleos de CPU, 4.096 M de memória, localizada no data center: dal10,
o sistema de operação é UBUNTU 16 de 64 bits, 2 discos, um de 100 G, o outro de 1.000 G e colocado na vlan pública com ID 413.
```
  bluemix sl vs create -H myvsi -D bluemix.net -c 4 -m 4096 -d dal10 -o UBUNTU_16_64 --disk 100 --disk 1000 --vlan-public 413 --test
```
Esse comando testa se a ordem é válida com as opções acima antes de ela ser realmente feita.
```
...
Esse comando exporta as opções acima para um arquivo: myvsi.txt sob o diretório inicial do usuário para uso posterior.


### bluemix sl vs options
{: #sl_vs_options}

Listar opções para criar uma instância de servidor virtual
```
...
```
 bluemix sl vs options
```
Esse comando lista todas as opções para criar uma instância de servidor virtual, por exemplo, data centers, cpu, memória, os, disco, velocidade de rede, etc.
...
Listar credenciais de instância de servidor virtual
```
...
```
Nota: se você não especificar argumentos, ele tentará atualizar os
registros A e de ponteiro. Se você não quiser atualizar os registros, poderá usar os
argumentos -a ou --ptr para limitar os registros atualizados.

<strong>Opções de comando</strong>:
   <dl>
   <dt>-a, --a-record</dt>
   <dd>Sincronizar o registro A do host</dd>
   <dt>--aaaa-record</dt>
   <dd>Sincronizar o registro AAAA do host</dd>
   <dt>--ptr</dt>
   <dd>Sincronizar o registro PTR do host</dd>
   <dt>--ttl</dt>
   <dd>Configura o TTL para os registros A e/ou PTR, o padrão é: 7.200</dd>
   <dt>-f, --force</dt>
   <dd>Forçar operação sem confirmação</dd>
   </dl>
...
```
Esse comando sincroniza o registro AAAA (endereço IP V6) e o registro PTR da instância de servidor virtual com ID 12345678 para o servidor DNS.
...
<strong>Opções de comando</strong>:
   ...
   <dt>--tag</dt>
   <dd>Tags para configurar ou sequência vazia para remover todas</dd>
   <dt>-u, --userdata</dt>
   <dd>Sequência de metadados definida pelo usuário</dd>
   <dt>-F, --userfile</dt>
   <dd>Ler dados do usuário do arquivo</dd>
...
```
Esse comando atualiza a instância de servidor virtual com ID `12345678` e configura seu domínio para ser `bluemix.net`, o nome do host para `myapp`, a tag para `testcli` e a velocidade da porta da rede pública para 1.000 Mbps.
...
<strong>Opções de comando</strong>:
   ...
   <dt>--tag</dt>
   <dd>Filtrar pelas tags (permitidas múltiplas ocorrências)</dd>
   <dt>--sortby</dt>
...
Ativar uma instância de servidor virtual
```
...
Verifique se uma instância de servidor virtual está pronta para uso
```
...
<strong>Opções de comando</strong>:
   ...
   <dd>Script de pós-instalação para download</dd>
   <dt>--image</dt>
   <dd>ID da imagem. O padrão é usar o sistema operacional atual.
Veja: 'bluemix sl image list' para obter referência</dd>
   <dt>-k, --key</dt>
   <dd>Os IDs das chaves SSH a serem incluídas no usuário raiz (permitidas múltiplas ocorrências)</dd>
   <dt>-f, --force</dt>
   <dd>Forçar operação sem confirmação</dd>
...
```
Esse comando recarrega o sistema operacional da imagem com ID 1234 para a instância de servidor virtual com ID 12345678.
...
```
Nota: o SoftLayer reinicializa automaticamente a instância depois de a solicitação de upgrade ser feita. A instância é interrompida até a conclusão da transação de upgrade. No entanto, para a Rede, nenhuma reinicialização é necessária.
...
```
Esse comando cria uma vlan localizada no roteador bcr01a.dal09 com 16 endereços IP e o nome é myvlan.
