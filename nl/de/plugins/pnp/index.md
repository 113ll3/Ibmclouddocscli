...
# Plug-in für privates Netzpeering für {{site.data.keyword.cloud_notm}}-Befehlszeilenschnittstelle
{: #private_network_cli}

Sie können die Befehlszeilenschnittstelle (CLI) für privates Netzpeering zum Konfigurieren und Verwalten von privatem Netzpeering zwischen zwei {{site.data.keyword.cloud}}-Bereichen verwenden. Privates Netzpeering wird für IBM Containers (Docker-Containers) unterstützt. Die {{site.data.keyword.cloud_notm}}-Bereiche können sich in unterschiedlichen Verfügbarkeitszonen derselben Region oder in verschiedenen Regionen befinden. Das CLI-Plug-in für privates Netzpeering steht für die Verwendung mit dem {{site.data.keyword.cloud_notm}}-CLI-Plug-in zur Verfügung.
...
Bevor Sie beginnen, erstellen Sie {{site.data.keyword.cloud_notm}}-Bereiche. Stellen Sie sicher, dass jeder Container eines Bereichs eine IP-Adresse aus einem anderen Netzwerk besitzt.

**Hinweis:** Wenn Sie das private Netzpeering mit einem {{site.data.keyword.cloud_notm}}-Bereich verwendet haben und den Bereich löschen müssen, dann löschen Sie zuerst die Verbindungen für das private Netzpeering in diesem Bereich.

Zu Beginn installieren Sie die {{site.data.keyword.cloud_notm}}-Befehlszeilenschnittstelle. Weitere Informationen finden Sie unter [IBM Cloud CLI](/docs/cli?topic=cloud-cli-ibmcloud-cli).
...
### Lokal installieren
Laden Sie das Plug-in für privates Netzpeering für Ihre Plattform aus dem [{{site.data.keyword.cloud_notm}}-CLI-Plug-in-Repository](https://plugins.cloud.ibm.com/ui/repository.html){: new_window} ![Symbol für externen Link](../../../icons/launch-glyph.svg "Symbol für externen Link") herunter.
...
Befolgen Sie diese Schritte, um das Plug-in aus dem {{site.data.keyword.cloud_notm}}-Repository zu installieren:

1. Fügen Sie den Registry-Endpunkt für das {{site.data.keyword.cloud_notm}}-Plug-in hinzu:
	```
