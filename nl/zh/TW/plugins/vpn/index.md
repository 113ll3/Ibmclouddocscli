...
下列指示適用於使用 Cloud Foundry (cf) CLI 外掛程式。若要搭配使用此外掛程式與 {{site.data.keyword.Bluemix_notm}} CLI 外掛程式，請參閱適用於 {{site.data.keyword.Bluemix_notm}} CLI 的 [{{site.data.keyword.vpn_short}} 外掛程式。

## 安裝 cf CLI 外掛程式
開始之前，請先安裝 cf CLI。如需詳細資料，請參閱 [Cloud Foundry 指令行介面](https://console.{DomainName}/docs/cli/downloads.html)(https://console.{DomainName}/docs/cli/plugins/bx_vpn/index.html)]。
...
1. 從 [IBM Bluemix CLI 外掛程式儲存庫](http://plugins.ng.bluemix.net/ui/repository.html#cf-plugins)，下載 {{site.data.keyword.vpn_short}} 外掛程式。
...
1. 將 Bluemix 儲存庫新增至 Cloud Foundry CLI 儲存庫。使用下列指令：
