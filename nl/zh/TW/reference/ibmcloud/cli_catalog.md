...
請使用下列指令管理 {{site.data.keyword.cloud}} 型錄項目、查詢範本、運行環境及資料中心的地理位置。
{: shortdesc}
...
<dl>
  ...
  <dt>-k, --kind</dt>
  <dd>依資源類型進行過濾。目前僅支援 "service-cf"、"iaas"、"runtime"、"template" 及 "dashboard"</dd>
  <dt>-p, --price</dt>
  <dd>依價格進行過濾。目前僅支援 "free"、"paygo"、"ibmcloud-subscription"</dd>
  <dt>-t, --tag</dt>
  <dd>依標籤進行過濾。</dd>
  <dt>--sort-by</dt>
  <dd>排序依據的內容</dd>
...
<strong>指令選項</strong>：
   <dl>
   <dt>TEMPLATE_ID（必要）</dt>
   <dd>應用程式在建立時所根據的範本。請使用 <i>ibmcloud templates</i> 來查看所有範本 ID。</dd>
   <dt>CF_APP_NAME（必要）</dt>
   <dd>要建立之 cf 應用程式的名稱。</dd>
   <dt>-n<i>HOSTNAME</i></dt>
   <dd>CF 應用程式的主機名稱。如果未指定，{{site.data.keyword.cloud_notm}} 會自動根據應用程式名稱及預設網域來設定路徑。</dd>
   <dt>-d<i>DOMAINNAME</i></dt>
   <dd>CF 應用程式的網域。如果未指定，{{site.data.keyword.cloud_notm}} 會自動根據應用程式名稱及預設網域來設定路徑。</dd>
