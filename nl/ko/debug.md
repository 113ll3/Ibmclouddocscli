...
다음 프로시저는 `cf logs` 명령을 사용하여 오류를 디버그하는 방법을 보여줍니다. 다음 단계를 수행하기 전에 Cloud Foundry 명령행 인터페이스를 설치했는지 확인하십시오. Cloud Foundry 명령행 인터페이스의 설치에 대한 자세한 정보는 [Cloud Foundry 명령행 인터페이스 설치 ![외부 링크 아이콘](../icons/launch-glyph.svg "외부 링크 아이콘")](/docs/starters/install_cli.html){: new_window}를 참조하십시오. 

  1. Cloud Foundry 명령행 인터페이스에 다음 코드를 입력하여 {{site.data.keyword.Bluemix_notm}}에 연결하십시오.
     ```
...
다음 예에서는 `cf logs appname --recent`를 입력하면 표시되는 로그를 보여줍니다. 이 예에서는 Node.js 애플리케이션에서 오류가 발생했다고 가정합니다. 
```
...
Cloud Foundry 명령행 인터페이스에 `cf logs appname --recent`를 입력하면 가장 최근의 로그만 표시됩니다. 이전에 발생한 오류에 대한 로그를 보려면 모든 로그를 가져와서 오류를 검색해야 합니다. 애플리케이션의 모든 로그를 가져오려면 다음 방법 중 하나를 사용하십시오.
<dl>
...
<dd>로그를 자동으로 수집하여 외부 파일로 내보내는 스크립트를 사용하려면 컴퓨터에서 {{site.data.keyword.Bluemix_notm}} 콘솔에 연결해야 합니다. 또한 컴퓨터에 로그를 다운로드할 수 있는 충분한 영역이 있어야 합니다. 자세한 정보는 <a href="/docs/get-support/quicktickresp.html#collecting-diagnostic-information" target="_blank">진단 정보 수집</a>을 참조하십시오. </dd>
</dl>
