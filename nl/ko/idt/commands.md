...
- [console](#console): 프로젝트에 대한 IBM Cloud 콘솔 열기
- [create](#create): 새 프로젝트를 작성하고 서비스를 추가할 수 있는 옵션 제공
- [debug](#debug): 로컬 컨테이너에 있는 애플리케이션 디버그
- [delete](#delete): 영역에서 프로젝트 삭제
- [deploy](#deploy): IBM Cloud에 애플리케이션 배치
- [enable](#enable): IBM Cloud 파일을 기존 프로젝트에 추가
- [get-credentials](#get-credentials): 바인딩된 서비스를 사용하기 위해 프로젝트에 필요한 신임 정보 가져오기
- [help](#help): IDT 구문 및 인수에 대한 도움말
- [list](#list): 영역 내의 모든 IBM Cloud 프로젝트 나열
...
- [view](#view): 테스트 및 보기를 위해 앱의 배치된 URL 보기
- [compound commands](#compound): 하나의 명령문에서 여러 명령 실행
...
여러 컨테이너를 빌드하기 위해서는 프로젝트가 `cli-config.yml`에 지정된 [Compose](https://docs.docker.com/compose/overview/) 파일을 포함해야 하거나, 사용자가 `dockerfile-tools` 명령 매개변수를 사용하여 이를 제공할 수 있습니다. 자세한 정보는 [Compose 파일](/docs/apps/projects/compose_file.html)을 참조하십시오.
...
`debug` 명령을 통해 애플리케이션을 디버그할 수 있습니다. 먼저 build 명령을 `--debug` 인수와 함께 사용하여 프로젝트에 대해 빌드를 완료해야 합니다. `debug` 명령을 시작하면 cli-config.yml의 `container-port-map-debug` 값으로 정의되거나 명령행에 지정된 디버그 포트를 제공하는 컨테이너가 시작됩니다. 원하는 디버깅 도구를 포트에 연결하면 일반적인 경우와 다름없이 애플리케이션을 디버그할 수 있습니다.
