...
# {{site.data.keyword.dev_cli_notm}}(bx dev) 명령
{: #idt-cli}

버전: 1.2.0
릴리스 날짜: 2018년 3월 8일

프로젝트를 작성하고 이를 배치, 디버그 및 테스트하려면 다음 {{site.data.keyword.dev_cli_notm}}(bx dev) 명령을 사용하십시오. 

- [build](#build): 로컬 컨테이너에 있는 프로젝트를 빌드함
- [code](#code): 프로젝트로부터 코드를 다운로드함
- [console](#console): 프로젝트에 대한 IBM Cloud 콘솔을 엶
- [create](#create): 새 프로젝트를 작성하고 서비스를 추가할 수 있는 옵션을 제공함
- [debug](#debug): 로컬 컨테이너에 있는 애플리케이션을 디버그함
- [delete](#delete): 영역에서 프로젝트를 삭제함
- [deploy](#deploy): IBM Cloud에 애플리케이션을 배치함
- [enable](#enable): IBM Cloud 파일을 기존 프로젝트에 추가함
- [get-credentials](#get-credentials): 바인딩된 서비스를 사용할 수 있도록 하기 위해 프로젝트에서 필요로 하는 신임 정보를 가져옴
- [help](#help): IDT 구문 및 인수에 대한 도움말
- [list](#list): 영역 내의 모든 IBM Cloud 프로젝트를 나열함
- [run](#run): 로컬 컨테이너에 있는 애플리케이션을 실행함
- [shell](#shell): 로컬 컨테이너에 대한 쉘을 엶
- [status](#status): CLI에 의해 사용된 컨테이너의 상태를 확인함
- [stop](#stop): 컨테이너를 중지함
- [test](#test): 로컬 컨테이너에 있는 애플리케이션을 테스트함
- [view](#view): 테스트 및 보기를 위해 앱의 배치된 URL을 봄
- [복합 명령](#compound): 하나의 명령행 명령문에서 여러 명령을 실행함
...
`build` 명령을 사용하여 애플리케이션을 빌드할 수 있습니다. `test`, `debug` 및 `run` 명령은 컴파일된 프로젝트를 찾을 것을 예상하므로 이러한 명령 전에 먼저 `build` 조작을 실행해야 합니다.   

`build-cmd-debug` 구성 요소는 `run`을 제외한 모든 용도를 위해 애플리케이션을 빌드하는 데 사용됩니다. 디버깅을 위해 애플리케이션을 빌드하는 경우에는 명령행 옵션 `--debug`를 지정합니다. `build-cmd-run` 구성 요소는 `run` 명령과 함께 사용하기 위해 애플리케이션을 빌드하는 경우 사용됩니다. 

여러 컨테이너를 빌드하기 위해서는 프로젝트가 `cli-config.yml`에 지정된 [Compose](https://docs.docker.com/compose/overview/) 파일을 포함해야 하며, 사용자는 `dockerfile-tools` 명령 매개변수를 사용하여 이를 제공할 수 있습니다. 자세한 정보는 [Compose 파일](/docs/apps/projects/compose_file.html)을 참조하십시오. 
...
{{site.data.keyword.Bluemix_notm}}의 애플리케이션 템플리트 코드 및 구성 파일을 사용하여 이전에 작성된 프로젝트를 다운로드하려면 `code` 명령을 사용하십시오. 이는 작성한 프로젝트의 두 번째 사본을 추출해야 하는 경우 유용합니다. 
...
선택적으로 프로젝트 이름을 제공할 수 있으며, 이렇게 하면 CLI가 폴더/애플리케이션 이름을 기반으로 하는 비교를 건너뜁니다. 이 경우 CLI는 이름 지정된 프로젝트의 콘솔을 웹 브라우저에서 엽니다.   
...
리소스 유형, 언어, 스타터 킷 및 DevOps 도구 체인 옵션을 포함한 모든 정보에 대한 프롬프트를 표시하며 프로젝트를 작성합니다. 프로젝트는 현재 디렉토리에 작성됩니다. 
...
`debug` 명령을 통해 애플리케이션을 디버그할 수 있습니다. 먼저 build 명령을 `--debug` 인수와 함께 사용하여 프로젝트에 대해 빌드를 완료해야 합니다. `debug` 명령을 호출하면 cli-config.yml의 `container-port-map-debug` 값으로 정의되거나 명령행에 지정된 디버그 포트를 제공하는 컨테이너가 시작됩니다. 원하는 디버깅 도구를 포트에 연결하면 일반적인 경우와 다름없이 애플리케이션을 디버그할 수 있습니다. 
...
{{site.data.keyword.Bluemix}} 영역에서 프로젝트를 제거하려면 `delete` 명령을 사용하십시오. 매개변수 없이 이 명령을 실행하여 사용 가능한 프로젝트를 나열한 후 번호 지정된 목록에서 삭제할 프로젝트를 선택할 수 있습니다. 프로젝트 코드 및 디렉토리는 로컬 디스크 공간에서 제거되지 않습니다. 
...
애플리케이션을 {{site.data.keyword.Bluemix}}에 Cloud Foundry 애플리케이션으로 배치하려면 프로젝트의 루트 디렉토리에 `manifest.yml` 파일이 있어야 합니다. 
...
cli-config.yml에서 이러한 항목을 정의하지 않은 경우에는 `-t container` 매개변수를 사용하여 배치해야 하며 이렇게 하면 모든 기타 값에 대한 프롬프트가 표시됩니다. 
...
다음 매개변수는 `deploy` 명령과 함께 사용하거나 프로젝트의 `cli-config.yml` 파일을 직접 업데이트하여 사용할 수 있습니다. 다른 명령과 공유하는 [추가 매개변수](#command-parameters)도 있습니다. 
...
{{site.data.keyword.Bluemix_notm}} 배치에 기존 프로젝트를 사용할 수 있도록 설정합니다. `enable` 명령은 기존 프로젝트의 언어를 자동으로 발견하려 시도한 후 필요한 추가 정보에 대한 프롬프트를 표시합니다. 이는 로컬 Docker 컨테이너, CloudFoundry 배치 또는 Kubernetes/컨테이너 배치에 사용할 수 있는 파일을 생성하고 추가합니다. 
...
필요한 파일들이 있으면 올바른 프로젝트 구조를 위한 프로젝트 언어 발견이 수행됩니다.   

* `package.json` 파일이 있는 경우 Node.js 프로젝트가 식별됩니다. 
* `package.swift` 파일이 있는 경우 Swift 프로젝트가 식별됩니다. 
* `setup.py` 또는 `requirements.txt` 파일이 있는 경우 Python 프로젝트가 식별됩니다. 
* `pom.xml` 또는 `build.gradle` 파일이 있는 경우 Java 프로젝트가 식별됩니다. 
	* `pom.xml`이 있는 경우 Maven 프로젝트가 식별됩니다. 
	* `build.gradle`이 있는 경우 Gradle 프로젝트가 식별됩니다. 

선택적으로 `--language` 인수를 사용하여 발견된 프로젝트 언어를 대체할 수 있습니다. 그러나 올바르며 완전한 프로젝트만 지원됩니다. enable 명령은 소스 코드를 수정하지 않습니다. 
...
다음 매개변수는 `enable` 명령과 함께 사용하거나 프로젝트의 `cli-config.yml` 파일을 직접 업데이트하여 사용할 수 있습니다. 다른 명령과 공유하는 [추가 매개변수](#command-parameters)도 있습니다. 
...
바인딩된 서비스를 사용할 수 있도록 설정하기 위해 프로젝트에서 필요로 하는 신임 정보를 가져옵니다. 
...
영역 내의 모든 {{site.data.keyword.Bluemix_notm}} 프로젝트를 나열할 수 있습니다. 
...
`run` 명령을 통해 애플리케이션을 실행할 수 있습니다. `build` 명령을 사용하여 프로젝트에 대해 빌드를 완료해야 합니다. `run` 명령을 실행하면 run 컨테이너가 시작되며 `container-port-map` 매개변수로 정의된 포트를 노출합니다. run 컨테이너 Dockerfile이 이 단계를 완료하는 데 필요한 시작점을 포함하지 않는 경우에는 애플리케이션을 호출하는 데 `run-cmd` 매개변수를 사용할 수 있습니다. 

여러 컨테이너에서 실행하기 위해서는 프로젝트가 `cli-config.yml`에 지정된 [Compose](https://docs.docker.com/compose/overview/) 파일을 포함해야 하며, 사용자는 `dockerfile-run` 명령 매개변수를 사용하여 이를 제공할 수 있습니다. 자세한 정보는 [Compose 파일](/docs/apps/projects/compose_file.html)을 참조하십시오. 
...
다음 매개변수는 `run` 명령 전용이며 run 컨테이너 내에서 애플리케이션을 관리하는 데 도움을 줍니다. 다른 명령과 공유하는 [추가 매개변수](#command-parameters)도 있습니다. 
...
`test` 명령을 통해 애플리케이션을 테스트할 수 있습니다. `build --debug` 명령을 사용하여 프로젝트에 대해 빌드를 완료해야 합니다. 그 후에는 애플리케이션에 대해 `test-cmd`를 호출하기 위해 tools 컨테이너가 사용됩니다. 
...
모든 플래그는 마지막 명령 뒤에 와야 하며 해당 플래그가 연관된 모든 명령에 적용됩니다. 위 예에서 `--trace` 플래그는 세 명령 모두에 적용 가능하지만 `-t`는 마지막 두 명령에만 적용 가능하므로 `build` 명령에는 적용되지 않습니다. 
...
다음 매개변수는 `build|debug|run|test` 명령과 함께 사용하거나 프로젝트의 `cli-config.yml` 파일을 직접 업데이트하여 사용할 수 있습니다. 추가 매개변수는 [`debug`](#debug-parameters) 및 [`run`](#run-parameters) 명령에 대해 사용 가능합니다. 
