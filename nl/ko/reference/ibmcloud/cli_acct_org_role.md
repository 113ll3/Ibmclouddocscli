...
<strong>명령 옵션</strong>:
   ...
   <dt>-c ACCOUNT_ID</dt>
   <dd>계정 ID. 계정의 조직을 나열합니다. 지정되지 않은 경우 현재 계정으로 기본값이 지정됩니다. 'all'로 설정되면 모든 계정 아래 조직이 나열됩니다. 이 옵션은 '-u'와 배타적입니다.</dd>
...
<strong>명령 옵션</strong>:
   ...
   <dt>--guid</dt>
   <dd>지정된 조직의 GUID를 검색하고 표시합니다. 조직의 기타 모든 출력은 억제됩니다. 이 옵션은 '--output'과 배타적입니다.</dd>
   <dt>--output REGION</dt>
   <dd>출력 형식을 지정합니다. 이제 JSON만 지원됩니다. 이 옵션은 '--guid'와 배타적입니다.</dd>
   </dl>
...
새 조직을 작성합니다. 이 조작은 계정 소유자만 실행할 수 있습니다.
```
...
<strong>명령 옵션</strong>:
   ...
   <dt>--guid</dt>
   <dd>지정된 영역의 GUID를 검색하고 표시합니다. 영역의 기타 모든 출력은 억제됩니다. 이 옵션은 '--output'과 배타적입니다.</dd>
   <dt>--output FORMAT</dt>
...
CLI를 사용하여 조직/영역 역할을 설정할 수 있지만 그 외의 권한을 설정하려는 경우에는 UI를 사용해야 합니다. 추가 세부사항은 [리소스에 대한 액세스 관리](/docs/iam?topic=iam-iammanidaccser)를 참조하십시오.
{: note}
<!-- Begin Staging URL vs Prod URL -->
...
액세스 권한을 지정할 준비가 되어 있지 않거나 Cloud Foundry 액세스 권한 대신 IAM 정책을 지정하려는 경우, 액세스 권한이 없는 사용자를 초대하고 나중에 액세스 권한을 지정할 수 있습니다. 사용자에게 액세스 권한을 지정하는 데 대한 자세한 정보는 [리소스에 대한 액세스 관리](/docs/iam?topic=iam-iammanidaccser#assign_new_access)를 참조하십시오.
{: tip}
